...
class UDPHandler(SocketServer.BaseRequestHandler):   
    ...
    def processInvite(self):
        logging.debug("-----------------")
        logging.debug(" pozvanka bola dorucena ")
        logging.debug("-----------------")
        ...
        if len(destination) > 0:
            ...
            self.sendResponse("500 Server Error")
                                                        # zapis do denniku hovorov
        with open("Dennik_hovorov.txt", "a") as dennik:
            print("\nOdchádzajúci hovor:",origin.split('@')[0],file=dennik)
            print("Prichádzajúci hovor:",destination.split('@')[0],file=dennik)

                
    def processAck(self):
        logging.debug("--------------")
        logging.debug(" ACK doručené ")
        logging.debug("--------------")
        destination = self.getDestination()
        if len(destination) > 0:
            logging.info("destination %s" % destination)
    ...
    def processCode(self):
        ...
        if len(origin) > 0:
            ...
            if origin in registrar:
                ...
                nova_hlaska = data[0].split(" ")
                if nova_hlaska[1] == "180":
                    nova_hlaska[2] = "Zvonenie"
                elif nova_hlaska[1] == "200":
                    nova_hlaska[2] = "OKEJ"
                elif nova_hlaska[1] == "100":
                    nova_hlaska[2] = "Skusanie"
                data[0] = " ".join(nova_hlaska)
                ...
                logging.info("<<< %s" % data[0])
                                            # identifikacia zaciatku hovoru
                global prev,start,now_z
                if data[0].__contains__("200") & (prev == 180):
                    start = timeit.default_timer()
                    now_z = datetime.datetime.now()
                    prev = 0
                elif data[0].__contains__("180"):
                    prev = 180
                else:
                    prev = 0
                logging.debug("---\n<< server send [%d]:\n%s\n---" % (len(text),text))
    ...
    def handle(self):
        ...
        if rx_request_uri.search(request_uri) or rx_code.search(request_uri):
            ...
            # print(request_uri)
                                        # zapis do dennika hovorov
            if request_uri.__contains__("BYE"):
                global end
                end = timeit.default_timer()
                now_k = datetime.datetime.now()
                with open("Dennik_hovorov.txt", "a") as dennik:
                    print(now_z.strftime("%Y-%m-%d %H:%M:%S") +" Zaciatok hovoru",file=dennik)
                    print(now_k.strftime("%Y-%m-%d %H:%M:%S Koniec hovoru"),file=dennik)
                    print("dlzka hovoru: {} sec".format(round(end-start)),file=dennik)
                    print(self.data[5],file=dennik)
            if request_uri.__contains__("CANCEL"):
                with open("Dennik_hovorov.txt", "a") as dennik:
                    print("\n",file=dennik)
